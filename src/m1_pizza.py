...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 1:
    expected = [rg.Point(100, 350),  # All numbers are approximate.
                rg.Point(150, 300),
                rg.Point(100, 250),
                rg.Point(50, 300)]
...
def run_test_draw_points_on_circle():
    ...
    circle = rg.Circle(rg.Point(500, 500), 250)
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'PIZZA test 4:  100 slices, thin (thickness=1) green lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 1
    pizza(window, circle, 100, 'green', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    polygon(window, circle, 7, 'magenta', 3)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(220, 200), 100)
    circle.fill_color = 'magenta'
    fancy_polygon(window, circle, 3, 7, 'green', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        lines = rg.Line(points[(k - (hops_to_next_point))%len(points)],points[k])
        lines.thickness = thickness
        lines.color = color
        lines.attach_to(window)
        window.render()
